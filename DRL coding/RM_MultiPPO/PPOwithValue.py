    ...
    def update(self,  memories, epoch):
        ...
        for _ in range(configs.k_epochs):
            ...
            for i in range(len(memories.fea_mb)):
                ...
                a_entropy, v, log_a, action_node, _, mask_mch_action, hx = self.policy_job(x=env_fea.to(device),
                                                                                           graph_pool=g_pool_step,
                                                                                           padded_nei=None,
                                                                                           adj=env_adj.to(device),
                                                                                           candidate=env_candidate.to(device)
                                                                                           , mask=env_mask.to(device)
                                                                                           , mask_mch=env_mask_mch.to(device)
                                                                                           , dur=env_dur.to(device)
                                                                                           , a_index=a_index
                                                                                           , old_action=old_action.to(device)
                                                                                           ,mch_pool=pool
                                                                                           , old_policy=False
                                                                                           )
