...
def validate(vali_set, batch_size, policy_jo, policy_mc):
    ...
    def eval_model_bat(bat, i):
        ...
        with torch.no_grad():
            ...
            while True:
                env_adj = aggr_obs(
                    deepcopy(adj).to(device).to_sparse(), configs.n_j * configs.n_m
                )
                ...
                if env.done():
                    ...
                    #plt.savefig("./1515_%s.svg"%i, format='svg',dpi=500, bbox_inches='tight')
                    break
    ...
    totall_cost = torch.cat(
        [eval_model_bat(bat, i) for i, bat in enumerate(vali_set)], 0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Pn_j", type=int, default=10, help="Number of jobs of instances to test"
    )
    parser.add_argument(
        "--Pn_m", type=int, default=5, help="Number of machines instances to test"
    )
    parser.add_argument(
        "--Nn_j",
        type=int,
    ...
    filepath = os.path.join(current_directory, "saved_network")
    ...
    num_val = 1 # validate sample number
    batch_size = 1
    SEEDs = [23, 56]
    result = []
    loade = False
    ...
    for SEED in SEEDs:
        ...
        np.random.seed(SEED)
        if loade:
            # file_path = "FJSP_J%sM%s_unew_test_data.npy" % (configs.n_j, configs.n_m)
            filepath = "F:\\BTicket\\rl_fjps\\FJSP\\FJSP_MultiPPO\\data\\history.npy"
            validat_dataset = np.load(
                file = filepath, allow_pickle=True
            )
            validat_dataset = dict(validat_dataset)
            # print(f"job:\n{validat_dataset['job']}operation:\n{validat_dataset['operation']}\nmach_a:\n{validat_dataset['mach_a']}\n")
            # print(f'start_time:\n{validat_dataset['start_time']}dur_a:\n{validat_dataset['dur_a']}\id2product_job:\n{validat_dataset['id2product_job']}\n')
            # print(f'nj_nm:\n{validat_dataset['nj_nm']}makespan:\n{validat_dataset['makespan']}')
        else:
            validat_dataset = FJSPDataset(
                configs.n_j, configs.n_m, configs.low, configs.high, num_val, SEED
            )
        ...
        print(f"mean time: {np.array(vali_result).mean()}\tscheme:\n{vali_result}")
