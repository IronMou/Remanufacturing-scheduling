...
class FJSP(gym.Env, EzPickle):
    ...
    def step(self, action, mch_a, gantt_plt=None):
        ...
        for i in range(self.batch_sie):
            ...
            if action[i] not in self.partial_sol_sequeence[i]:
                ...
                # temp1.shape()
                self.LBs[i] = calEndTimeLB(
                    self.temp1[i], self.input_min[i], self.input_mean[i]
                )
            ...
            fea = np.concatenate(
                (
                    ...
                    self.finished_mark[i].reshape(-1, 1),
                ),
                axis=-1,
            )
    ...
    def reset(self, data):
        ...
        for i in range(self.batch_sie):
            ...
            first_col = np.arange(start=0, stop=self.number_of_tasks, step=1).reshape(
                self.number_of_jobs, -1
            )[:, 0]
            self.first_col.append(first_col)
            # the task id for last column
            last_col = np.arange(start=0, stop=self.number_of_tasks, step=1).reshape(
                self.number_of_jobs, -1
            )[:, -1]
        ...
        self.step_count = 0
        self.m = -1 * np.ones(
            (self.batch_sie, self.number_of_jobs, self.number_of_machines),
            dtype=np.int64,
        )
        ...
        for t in range(self.batch_sie):
            ...
            for i in range(self.number_of_jobs):
                ...
                for j in range(self.number_of_machines):
                    ...
                    self.dur[t][i][j] = [
                        durmch.mean() if i <= 0 else i for i in self.dur[t][i][j]
                    ]
        ...
        self.input_2d = np.concatenate(
            [
                self.input_min.reshape(
                    (self.batch_sie, self.number_of_jobs, self.number_of_machines, 1)
                ),
        ...
        fea = np.concatenate(
            (
                ...
                ,
                self.finished_mark.reshape(self.batch_sie, -1, 1),
            ),
            axis=-1,
        )
        # --------------------------------------------------------------------------------------------------------------------------
        ...
        self.mask_mch = self.mask_mch.reshape(
            self.batch_sie, -1, self.mask_mch.shape[-1]
        )
        return (
            self.adj,
            fea,
...
class DFJSP_GANTT_CHART:
    ...
    def init_tiff(self):
        ...
        pic_dir = os.path.join(current_directory, "FJSP_FIGURE", "pic")
        if not os.path.exists(pic_dir):
            os.makedirs(pic_dir, exist_ok=True)
        self.filename = f"{pic_dir}/J{self.total_n_job}_ma{self.number_of_machines}.tif"
    ...
    def save_to_tiff(self):
        ...
        if len(self.frames) > 1:
            self.frames[0].save(self.filename, save_all=True, append_images=self.frames[1:], compression='tiff_lzw')
