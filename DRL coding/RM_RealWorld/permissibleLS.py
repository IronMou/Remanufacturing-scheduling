...
def calJobAndMchRdyTimeOfa(
    ...
    if jobPredecessor is not None:
        ...
        # opIDsOnMchs->对应mchJobPredecessor----shape（machine,n_job）
        # 找到数组opIDsOnMchs[mchJobPredecessor]中等于jobPredecessor的索引值
        # opIDsOnMchs->shape(machine, job)
        jobRdyTime_a = (
...
def permissibleLeftShift(
    ...
    # machine中以调度的task的start_time大于job中action的上一个task的finish_time
    ...
    def runDRs(rule):
        ...
        for batch_idx, data_set in enumerate(data_loader):
            ...
            while True:
                ...
                adj, _, reward, done, omega, mask, job, mch_mask, mch_time, _ = (
