...
class Job_Actor(nn.Module):
    ...
    def forward(
        ...
        if old_policy:
            ...
            batch_node = h_nodes.reshape(dummy.size(0), -1, dummy.size(-1)).to(
                self.device
            )
            ...
            batch_x = dur.reshape(dummy.size(0), -1, self.n_m).to(self.device)
            ...
            mask_mch_action = torch.gather(
                ...
                action1.unsqueeze(-1)
                .unsqueeze(-1)
                .expand(mask_mch.size(0), -1, mask_mch.size(2)),
            )
            # --------------------------------------------------------------------------------------------------------------------
            action_feature = torch.gather(
                ...
                action1.unsqueeze(-1)
                .unsqueeze(-1)
                .expand(batch_node.size(0), -1, batch_node.size(2)),
            ).squeeze(1)
            action_node = torch.gather(
                ...
                action1.unsqueeze(-1)
                .unsqueeze(-1)
                .expand(batch_x.size(0), -1, batch_x.size(2)),
            ).squeeze()  # [:,:-2]
            ...
            return (
                ...
                log_a,
                action_node.detach(),
                action_feature.detach(),
                mask_mch_action.detach(),
                h_pooled.detach(),
            )
            ...
            batch_x = dur.reshape(dummy.size(0), self.n_j * self.n_m, -1).to(
                self.device
            )
...
class Mch_Actor(nn.Module):
    def __init__(
        ...
        # self.input = nn.Parameter(torch.Tensor( hidden_dim)).to(device)
        # self.input.data.uniform_(-1, 1)
        ...
        # self.pointer = Pointer(hidden_dim,1).to(device)
        # self.mlp_encoder = MLP(3,2,hidden_dim,hidden_dim).to(device)
    ...
    def forward(
        ...
        # action_node = self.mlp_encoder(feature)
        action_node = self.bn(self.fc2(feature).reshape(-1, self.hidden_size)).reshape(
            -1, self.n_m, self.hidden_size
        )
        ...
        concateFea = torch.cat(
            (action_node, h_pooled_repeated, pooled_repeated), dim=-1
        )
        ...
        mch_scores = mch_scores.masked_fill(
            mask_mch_action.squeeze(1).bool(), float("-inf")
        )
