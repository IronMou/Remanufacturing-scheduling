        ...
        if old_policy:
            ...
            if mch_pool == None:
                mch_pooled_repeated = (
                    ...
                    .expand_as(candidate_feature)
                    .to(self.device)
                )
            else:
                mch_pooled_repeated = (
                    mch_pool.unsqueeze(-2).expand_as(candidate_feature).to(self.device)
                )
            concateFea = torch.cat(
                (candidate_feature, h_pooled_repeated, mch_pooled_repeated), dim=-1
            )
