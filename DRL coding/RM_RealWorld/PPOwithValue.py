...
class PPO:
    ...
    def update(self, memories, epoch):
        ...
        for _ in range(3):
            ...
            job_entropy = []
            mch_entropies = []
            job_scheduler = LambdaLR(
                self.job_optimizer, lr_lambda=lambda f: 0.96**epoch
            )
            mch_scheduler = LambdaLR(
                self.mch_optimizer, lr_lambda=lambda f: 0.96**epoch
            )
            ...
            for i in range(len(memories.fea_mb)):
                ...
                a_entropy, v, log_a, action_node, _, mask_mch_action, hx = (
                    self.policy_job(
                        x=env_fea,
                ...
                pi_mch, _, last_hh = self.policy_mch(
                    action_node,
                    hx,
                    mask_mch_action,
                    env_mch_time,
            ...
            job_log_prob, job_log_old_prob = torch.stack(job_log_prob, 0).permute(
                1, 0
            ), torch.stack(job_log_old_prob, 0).permute(1, 0)
            mch_log_prob, mch_log_old_prob = torch.stack(mch_log_prob, 0).permute(
                1, 0
            ), torch.stack(mch_log_old_prob, 0).permute(1, 0)
            ...
            for j in range(configs.batch_size):
                ...
                job_surr2 = (
                    torch.clamp(job_ratios, 1 - self.eps_clip, 1 + self.eps_clip)
                    * advantages
                )
                job_v_loss = self.MSE(val[j], rewards_all_env[j])
                job_loss = (
                    -torch.min(job_surr1, job_surr2)
                    + 0.5 * job_v_loss
                    - 0.01 * job_entropy[j]
                )
                ...
                mch_surr2 = (
                    torch.clamp(mch_ratios, 1 - self.eps_clip, 1 + self.eps_clip)
                    * advantages
                )
                # mch_v_loss = self.MSE(val[j], rewards_all_env[j])
                mch_loss = (
                    -torch.min(mch_surr1, mch_surr2)
                    - 0.01 * mch_entropies[j]
                    + 0.5 * job_v_loss
                )
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            while True:
                ...
                action, a_idx, log_a, action_node, _, mask_mch_action, hx = (
                    ppo.policy_old_job(
                        x=env_fea,
                        graph_pool=g_pool_step,
                        padded_nei=None,
