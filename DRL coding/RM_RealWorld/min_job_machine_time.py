...
def min_mch_job(mch_time,mchsEndTimes,number_of_machines,dur,temp,omega):
    ...
    while True:
        ...
        for z in range(len(min_mch_time)):
            ...
            task = np.intersect1d(min_mchForJob[z],omega)
            if len(task) == 0:
...
def min_job_mch(mch_time,job_time, mchsEndTimes, number_of_machines, dur, temp, omega,mask_last,done,mask_mch,first_col,last_col):
    temp = np.copy(temp)
    for machine, j in zip(mchsEndTimes, range(number_of_machines)):
        if np.all(machine == -configs.high):
            mch_time[j] = 0
