...
def lastNonZero(arr, axis, invalid_val=-1):
    ...
    val = (
        arr.shape[axis] - np.flip(mask, axis=axis).argmax(axis=axis) - 1
    )  # 按指定轴翻转数组中的元素
    # 如果最后一个元素不为0或全为0则np.flip(mask, axis=axis).argmax(axis=axis)等于0
    yAxis = np.where(
        mask.any(axis=axis), val, invalid_val
    )  # np.any()任意元素为true即输出true#三个参数np.where(cond,x,y)：满足条件（cond）输出x，不满足输出y;cond为bool矩阵
...
def calEndTimeLB(temp1, min, mean):
    ...
    temp2 = np.concatenate(
        (
            temp20.reshape(temp20.shape[0], temp20.shape[1], 1),
            temp21.reshape(temp20.shape[0], temp20.shape[1], 1),
        ),
        -1,
    )
