...
class DFJSP_GANTT_CHART:
    ...
    def colour_gen(self, n):
        ...
        for i in range(n):
            colour_bits = ["#"]
            colour_bits.extend(random.sample(global_color_bits, 6))
    ...
    def init_tiff(self):
        ...
        self.pic_dir = os.path.join(current_directory, "FJSP_FIGURE")
...
class FJSP(gym.Env, EzPickle):
    def __init__(
        self, n_j, n_m, EachJob_num_operation, product_num_jobs=None, need_gantt=False
    ):
        ...
        self.need_gantt = need_gantt
        if need_gantt:
            self.gantt = DFJSP_GANTT_CHART(
                total_n_job=n_j, number_of_machines=n_m, product_arr=self.product_arr
            )
    ...
    def reset(self, data, rule):
        ...
        self.mchMat = -1 * np.ones(
            (self.batch_sie, self.number_of_jobs, self.max_operation), dtype=np.int32
        )

        self.dur = data.astype(np.float32)  # single单精度浮点数
        self.dur_cp = deepcopy(self.dur)
        # record action history

        self.partial_sol_sequeence = -1 * np.ones(
            (self.batch_sie, self.number_of_tasks), dtype=np.int32
        )
        ...
        for t in range(self.batch_sie):
            ...
            for i in range(self.number_of_jobs):
                ...
                for j in range(self.max_operation):
                    ...
                    self.dur[t][i][j] = [
                        100 if i <= 0 else i for i in self.dur[t][i][j]
                    ]
                    if len(durmch) == 0:
                        dur_min.append(1)
                        dur_mean.append(1)
